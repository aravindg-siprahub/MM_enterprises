...
from app.schemas.product import Product, ProductCreate, ProductUpdate, ProductImageCreate, ProductImage
...
def update_product_route(product_id: str, product: ProductUpdate, db: tuple = Depends(get_db)):
    conn, cursor = db
        ...
        if images_data is not None:
            ...
            for ext_url, ext_id in existing_urls.items():
                if ext_url not in new_urls:
                    delete_from_supabase("products", ext_url)
...
def delete_product(product_id: str, db: tuple = Depends(get_db), current_user: dict = Depends(get_current_admin)):
    conn, cursor = db
