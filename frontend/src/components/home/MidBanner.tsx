...
export default function MidBanner({ banner }: { banner: any }) {
  ...
  return (
    <Link href={banner.link_url || banner.cta_link || '#'}
          className="relative block w-full overflow-hidden rounded-sm shadow-sm
                     cursor-pointer hover:opacity-95 transition-opacity group"
      ...
      <div className="absolute inset-0 bg-gradient-to-r 
                      from-black/65 via-black/20 to-transparent z-10" />
      <div className="absolute inset-0 flex flex-col justify-center 
                      pl-6 sm:pl-12 pr-[45%] z-20">
        ...
        {banner.cta_text && (
          <span className="inline-block bg-white text-[#2874f0] font-bold
                           text-xs sm:text-sm px-3 sm:px-5 py-1.5 sm:py-2
                           rounded w-fit group-hover:bg-[#2874f0] group-hover:text-white
