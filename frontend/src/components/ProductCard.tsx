  ...
  return (
    ...
    >
      ...
      <div className="absolute top-4 left-4 z-20 flex flex-col gap-2">
        {product.is_featured && (
          <span className="bg-primary/90 text-primary-foreground text-[10px] font-bold px-2 py-1 rounded-md uppercase tracking-wider backdrop-blur-md">
            Featured
          </span>
        )}
        {discount > 0 && (
          <span className="bg-destructive/90 text-destructive-foreground text-[10px] font-bold px-2 py-1 rounded-md backdrop-blur-md">
            {discount}% OFF
          </span>
        )}
      </div>
      ...
      <div className="absolute top-4 right-4 z-20 flex flex-col gap-2">
        <motion.button 
          ...
          className="p-2 bg-background/80 hover:bg-background backdrop-blur-md rounded-full shadow-sm text-muted-foreground transition-colors"
        >
          <Heart className={`w-4 h-4 ${isWishlisted ? 'fill-destructive text-destructive' : 'hover:text-destructive'}`} />
        </motion.button>
        ...
        <AnimatePresence>
          {isHovered && (
            <motion.button
              ...
              exit={{ opacity: 0, x: 10 }}
              className="p-2 bg-background/80 hover:bg-background backdrop-blur-md rounded-full shadow-sm text-muted-foreground hover:text-primary transition-colors hidden sm:block"
            >
